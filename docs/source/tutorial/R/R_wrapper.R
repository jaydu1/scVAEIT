...
#' @param masks An n-by-p binary matrix indicating the missing entries. -1 for missing entries, 0 for observed entries.
...
#' @param num_epoch The number of training epochs.
...
vae <- function(
  ...
  block_names=NULL, uni_block_names=NULL, dim_block_embed=128L, skip_conn=TRUE,
  num_epoch=20L, num_repeat=500L, batch_size=128L, batch_size_inference=512L,
  # hyperparameters for training
  save_every_epoch=NULL,
  beta_kl=1., beta_unobs=0.5, beta_reverse=0.2, beta_modal=NULL, p_feat=0.5, p_modal=NULL, seed=0,
  ...
  X_imp <- model$get_denoised_data(return_mean=return_mean, L=L, batch_size_inference=batch_size_inference)
