...
def get_dist(dist):
    if dist=='NB':
        generative_dist = lambda x_hat, disp, mask, zi_prob, mean_val, min_val, max_val: tfd.Independent(tfd.Masked(
        ...
        def generative_dist(x_hat, disp, mask, zi_prob, mean_val, min_val, max_val): 
            return tfd.Independent(tfd.Masked(tfd.Normal(
                loc = tfp.math.clip_by_value_preserve_gradient(x_hat + mean_val, min_val, max_val),
                scale = disp, name='Gaussian_rv'
                ), mask), reinterpreted_batch_ndims=1)
    elif dist=='Poisson':
        generative_dist = lambda x_hat, disp, mask, zi_prob, mean_val, min_val, max_val: tfd.Independent(tfd.Masked(
            tfd.Poisson(
                log_rate = tfp.math.clip_by_value_preserve_gradient(
                    x_hat, 
                    tf.where(min_val > 0, tf.math.log(min_val), tf.constant(-np.inf)), 
                    tf.math.log(max_val)), 
                name='Poisson_rv'
            ), mask), reinterpreted_batch_ndims=1)
    elif dist=='ZINB':
...
class OutputBlock(tf.keras.layers.Layer):
    def __init__(self, dim_outputs, dist_outputs, dim_latents, dim_embed, 
        ...
        **kwargs : 
        ...
        self.linear_layers = [
            Dense(d, activation = LeakyReLU(), name=names[i]) if d>0 else 
        ...
        self.output_layers = [Dense(d, name=names[i]) for i,d in enumerate(self.dim_outputs)]
        self.out_act = [tf.identity if dist in ['Poisson','Gaussian'] else 
        ...
        self.max_vals = tf.split(tf.expand_dims(tf.expand_dims(max_vals, 0), 0), self.dim_outputs, axis=-1)
        self.min_vals = tf.split(tf.expand_dims(tf.expand_dims(min_vals, 0), 0), self.dim_outputs, axis=-1)
        self.mean_vals = tf.split(tf.expand_dims(tf.expand_dims(mean_vals, 0), 0), self.dim_outputs, axis=-1)
        ...
        self.dists = [get_dist(dist) for dist in self.dist_outputs]
    ...
    def call(self, x, embed, masks, batches, z, x_embed, training=True):
        ...
        m_list = tf.split(tf.expand_dims(masks,1), self.dim_outputs, axis=-1)
        x_list = tf.split(tf.expand_dims(x,1), self.dim_outputs, axis=-1)
        ...
        probs = self.concat([
            self.dists[i](
                ...
                self.mean_vals[i], self.min_vals[i], self.max_vals[i]
...
class Sampling(Layer):
    ...
    def call(self, z_mean, z_log_var):
        ...
        z : tf.Tensor
            \([B, L, d]\) The sampled \(z\).
        '''   
        epsilon = self.g.normal(shape = tf.shape(z_mean), dtype=tf.keras.backend.floatx())
